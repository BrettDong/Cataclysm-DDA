...
TranslationDocument::TranslationDocument( const std::string &path )
{
    ...
    if( data.size() < 4 ) {
        throw InvalidTranslationDocumentException( path, "file is too small" );
    }
    if( GetByte( 0 ) == 0x95U &&
        GetByte( 1 ) == 0x04U &&
        GetByte( 2 ) == 0x12U &&
    ...
    if( original_strings_table_offset >= data.size() ) {
        throw InvalidTranslationDocumentException( path, "original strings table offset out-of-bound" );
    }
    if( translated_strings_table_offset >= data.size() ) {
        throw InvalidTranslationDocumentException( path, "translated strings table offset out-of-bound" );
    }
    for( std::size_t i = 0; i < number_of_strings; i++ ) {
        ...
        if( offset + length >= data.size() ) {
            throw InvalidTranslationDocumentException( path,
                    string_format( "original string %zu offset out-of-bound", i ) );
        }
        if( data[offset + length] != '\0' ) {
            throw InvalidTranslationDocumentException( path,
                    string_format( "original string %zu not terminated by '\\0'", i ) );
        }
    ...
    for( std::size_t i = 0; i < number_of_strings; i++ ) {
        ...
        if( offset + length >= data.size() ) {
            throw InvalidTranslationDocumentException( path,
                    string_format( "translated string %zu offset out-of-bound", i ) );
        }
        if( data[offset + length] != '\0' ) {
            throw InvalidTranslationDocumentException( path,
                    string_format( "translated string %zu offset out-of-bound", i ) );
